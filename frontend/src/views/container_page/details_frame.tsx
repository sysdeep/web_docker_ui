...
export default function DetailsFrame({ container }: DetailsFrameProps) {
  const ports_view = Object.keys(container.network.ports).map((port_name, idx) => {
          ...
          <table className='table table-small'>
            <tbody>
              <tr>
                <td>Image</td>
                <td>
                  <Link to={join_url(route.image, container.container.image)}>{container.config.image}</Link>
                </td>
              </tr>
              <tr>
                <td>Ports</td>
